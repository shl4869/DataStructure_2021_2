...
int duration = 30; // Simulation time
...
int remove_customer(int banker)
{
	element customer;
	int service_time = 0;
	...
	printf("Customer %d starts service in %d minutes.The banker is %d. Wait time was %d minutes.\n", customer.id, clock, banker, clock - customer.arrival_time);
	return service_time;
}
...
void main()
{
	...
	while (clock < duration) {
		...
		// Check if the customer who is receiving the service is finished.
		if (service_time2 > 0)	// the customer is receiving service
			service_time2--;
